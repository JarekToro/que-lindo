...
- [ ] Skim `git log` for anything you would not want public. History has 95
      ...
      personal paths.
- [ ] Decide whether to keep the vendored design-review skill (about 130 files,
      1.5 MB) committed. It is a design-review tool for Claude Code sessions,
      committed on purpose per `.gitignore`. Contributors who do not use it
      will see it as noise; the hook in `.claude/settings.json` is a no-op
      when the folder is absent, so it can be unvendored safely.
...
- [ ] `PRODUCT.md` and `DESIGN.md` are generated by the design tool and
      read like specs for an agent. Fine for contributors, but consider
      trimming or moving them under `docs/`.
